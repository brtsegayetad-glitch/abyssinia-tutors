...
function getAdminDbInstance() {
  if (!adminDb) {
    try {
      ...
      if (import_fs.default.existsSync(configPath)) {
        const firebaseConfig = JSON.parse(import_fs.default.readFileSync(configPath, "utf8"));
        const app = import_firebase_admin.default.apps[0] || import_firebase_admin.default.initializeApp({
          projectId: firebaseConfig.projectId
        });
        const dbId = firebaseConfig.firestoreDatabaseId;
        adminDb = dbId && dbId !== "(default)" ? (0, import_firestore2.getFirestore)(app, dbId) : (0, import_firestore2.getFirestore)(app);
...
async function sendFormattedEmail(to, subject, body, role) {
    ...
    const html = `
      ...
      return `<p style="margin-bottom: 16px;">${trimmed}</p>`;
        ...
        <div style="margin-top: 48px; padding-top: 24px; border-top: 1px solid #f1f5f9; text-align: center;">
          ...
          <p style="font-size: 10px; color: #cbd5e1; margin: 0;">&copy; ${(/* @__PURE__ */ new Date()).getFullYear()} Heritage Academy. All rights reserved.</p>
        </div>
      </div>
    `;
...
async function checkAndDispatchReminders() {
  ...
  try {
    ...
    const targetLimit = new Date(now.getTime() + 24 * 60 * 60 * 1e3);
    const snapshot = await adminDbInstance.collection("sessions").where("status", "==", "scheduled").get();
    let sentCount = 0;
    for (const sessionDoc of snapshot.docs) {
      ...
      if (session.startTime && typeof session.startTime.toDate === "function") {
        ...
        startDate = new Date(seconds * 1e3 + nanoseconds / 1e6);
      ...
      const isWithin24Hours = diffMs > 0 && diffMs <= 24 * 60 * 60 * 1e3;
      if (isWithin24Hours) {
        console.log(`[CRON] Found active session ${sId} starting in ${Math.round(diffMs / (60 * 1e3))} minutes. Resolving emails...`);
...
async function startServer() {
  ...
  app.post("/api/notify/email", async (req, res) => {
    ...
    try {
      ...
      res.json({
        success: true,
        message: result.simulated ? `Professional ${role || "user"} notification simulated for ${to} (Fallback)` : `Professional mail sent to ${to} via SMTP`,
